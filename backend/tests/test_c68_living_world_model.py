...
class TestLivingWorldModel:
    ...
    async def _observe(self, name="AI Agent Worker", category="role", domain="edu_tech", count=4):
        wm = get_world_model()
    ...
    async def test_emergence_lifecycle_and_provenance(self):
        ...
        await wm.recognize(key, evidence_status="verified", reviewer="reviewer-1")
    ...
    async def test_unverified_or_synthetic_cannot_be_recognized(self):
        ...
        with pytest.raises(ValueError):
            await wm.recognize(key, evidence_status="observed")
        wm2 = get_world_model()
        await wm2.observe("Fake Agent", "role", "demo", synthetic=True,
                    signal_data={"is_synthetic": True}, evidence_id="ev-s1")
        with pytest.raises(ValueError):
            await wm2.recognize("role:fake agent", evidence_status="verified")

    async def test_proposal_requires_recognized_and_governance_actor(self):
        wm, key = await self._observe()
        with pytest.raises(ValueError):
            await wm.propose(key, proposed_by="system")
        await wm.recognize(key, evidence_status="verified", reviewer="reviewer-1")
        proposal = await wm.propose(key, proposed_by="reviewer-1", reason="new role observed")
        assert proposal.status == "pending"
        assert wm.get_candidate(key).status == "proposed"
        with pytest.raises(ValueError):
            await wm.propose(key, proposed_by="reviewer-1")

    async def test_approval_requires_law_governance_and_human_actor(self):
        wm, key = await self._observe()
        await wm.recognize(key, evidence_status="verified", reviewer="reviewer-1")
        proposal = await wm.propose(key, proposed_by="reviewer-1")
        with pytest.raises(ValueError):
    ...
    async def test_adoption_does_not_mutate_registry(self):
        wm, key = await self._observe(name="AI Employee", domain="edu_tech")
        await wm.recognize(key, evidence_status="verified", reviewer="reviewer-1")
        proposal = await wm.propose(key, proposed_by="reviewer-1")
        await wm.review_proposal(proposal.proposal_id, "reviewer-2", "approved")
        adopted = await wm.adopt(proposal.proposal_id, "system_admin")
    ...
    async def test_industry_context_model(self):
        wm, key = await self._observe(domain="edu_tech")
        ctx = await wm.assess_industry("edu_tech", name="Education Technology")
