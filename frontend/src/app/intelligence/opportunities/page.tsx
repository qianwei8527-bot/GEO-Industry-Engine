...
export default function OpportunitiesPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <h1 className="text-3xl font-bold text-slate-100 mb-2 flex items-center gap-2"><Lightbulb className="w-6 h-6 text-yellow-400" /> 机会发现</h1>
      <p className="text-slate-400 mb-8">AI自动发现产业机会窗口、市场空白点、新兴赛道</p>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {data.map((c: any) => (
            <div key={c.id} className="bg-slate-900 border border-slate-800 rounded-lg p-4">
              ...
              <div className="text-xs text-slate-500 mt-1">行业: {c.description || "分析中..."} | 机会指数: 待评估</div>
            </div>
          ))}
        </div>
      )}
