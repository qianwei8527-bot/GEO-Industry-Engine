...
related_docs: [CTO长期开发协议, 02, 05-6, 06-3]
---
...
数据不是静态资产，需要从生到死的管理。详见 [32_GEO数据飞轮与生命周期.md](32_GEO数据飞轮与生命周期.md) 第四至七章。

| 概念 | 说明 | 关联层 |
|------|------|-------|
| 数据生命周期(5阶段) | 发现→验证→结构化→应用→反馈 | 数据湖全层 |
| 数据资产分层(4层) | 公共数据→关系数据→行为数据→智能数据 | 按访问控制区分 |
| 数据血缘追踪 | 每条数据记录来源链+可信等级 | 元数据层 |
